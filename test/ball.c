...
void gameLoop(){
    ...
    DrawCircle(GetMouseX(), GetMouseY() * -1 + GetScreenHeight(), 50, CLITERAL(Color){0,0,255,255});
    EndTextureMode();
    DrawTexture(rt.texture, 0, 0, CLITERAL(Color){255,255,255,255});
    DrawFPS(0, 0);
    DrawText(TextFormat("MouseX: %d", GetMouseX()), 0, 15, 20, WHITE);
    DrawText(TextFormat("MouseY: %d", GetMouseY() * -1 + 1080), 0, 30, 20, WHITE);
    #ifdef __EMSCRIPTEN__
